...
export default function Kameties({ totalEntries = 12, entriesPerPage = 10 }) {
  ...
  const handleGetAllKametis = async (page = 1, query = "", order = "desc",timePeriod = "") => {
    console.log("Sorting Order:", order); // Debugging: Check if order is passed correctly
    setScreenLoader(true);

    try {
      const response = await axios.post(
        `${apiBaseUrl}getAllKametis`,
        { page: page, perPage: perPage, search: query, sortOrder: order,timePeriod }, // ✅ Include `sortOrder`
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (response?.data?.status === true) {
        let newUsers = response?.data?.data?.committees || [];
  ...
  const handleSearchKametis = async (searchQuery, page = 1) => {
    setScreenLoader(true);
    try {
      const response = await axios.post(
        `${apiBaseUrl}getAllKametis`, // Same API endpoint
        {
          page: page,
      ...
      if (response?.data?.status === true) {
        ...
        setGetAllKametis(response?.data?.data?.committees || []); // Filtered data
  ...
  useEffect(() => {
    if (!searchQuery) {
      handleGetAllKametis(currentPage, "", sortOrder,selectedTimePeriod);
    }
  ...
  const handlePageChange = (newPage) => {
    ...
    handleGetAllKametis(newPage, searchQuery, "desc", selectedTimePeriod).finally(() => {
      setIsLoading(false);
    });
  };
  ...
  const handleCheckboxChange = async (option) => {
    ...
    setFilterOptions((prev) => {
      ...
      if (!prev[option]) {
        ...
        handleGetAllKametis(1, searchQuery, "desc", option);
      }
  ...
  const handleSearchChange = (e) => {
    ...
    const newTimeout = setTimeout(() => {
      handleSearchKametis(value, 1); // Send lowercase search term
    }, 500);
  ...
  <td colSpan="7" className="text-center align-middle py-12 text-gray-400">
    No records found
                ...
                {/* <KametiesTable Kameties={getAllKametis} handleGetKameti={handleGetAllKametis } setCurrentPage={setCurrentPage} /> */}
              </div>
            </div>
          </div>
        </div>
